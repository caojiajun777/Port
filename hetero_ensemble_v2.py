"""PortMoEv2-Hetero Ensemble v2: 文件级划分 + sheet_index 特征

改进:
  1. 文件级数据划分 — 同一条船所有快照保持在同一 split，杜绝数据泄露
  2. sheet_index 作为 conditioning 特征 — 告诉模型"这是该船第几个快照"
  3. 异方差高斯损失 × 4 种子 × TTA = 最强组合

使用方法:
    python hetero_ensemble_v2.py
"""
...
SAVE_DIR = EXPERIMENTS_DIR / "PortMoEv2-Hetero-Ensemble-v2"
...
def load_data_and_split():
    ...
    # ---- 文件级划分 (杜绝同一条船的快照泄露到不同 split) ----
    unique_files = np.unique(file_ids.numpy())
    n_files = len(unique_files)
    rng = np.random.RandomState(42)
    perm_files = rng.permutation(n_files)

    n_train_f = int(n_files * 0.8)
    n_val_f = int(n_files * 0.1)
    ...
    print(f"  文件级划分: {n_files} files → train={len(train_files)} val={len(val_files)} test={len(test_files)}")
...
def train_single_seed(seed, d, device):
    ...
    print(f"\n{'=' * 50}")
    print(f"  Seed {seed} — Hetero v2 训练 ({CFG['epochs']} epochs)")
    print(f"{'=' * 50}")
...
def ensemble_evaluate(seeds, d, device):
    ...
    print(f"  最终对比")
    print(f"{'=' * 60}")
    print(f"v1 (样本级划分, 无sheet_idx):    Test MAE_TEU=5.1112  R²=0.9550")
    print(f"v2 (文件级划分 + sheet_idx):     Test MAE_TEU={test_mae[0]:.4f}  "
...
def main():
    ...
    print("=" * 60)
    print("  PortMoEv2-Hetero Ensemble v2")
    print("  改进: 文件级划分 + sheet_index conditioning")
    print("=" * 60)
