"""PortMoEv2-Hetero 时序增强版 (v5)

核心创新: 时序上下文输入
  - 每个样本不再是独立的单帧快照
  - 而是堆叠当前帧 + 前2帧 → (18, 7, 22) 输入
  - 模型可以学习堆场变化模式(增减趋势、作业进程阶段)
  - 对 qc=1 (内在噪声大) 的改善潜力最大

其余训练配置与 v1 (hetero_ensemble.py) 完全一致:
  - 异方差高斯损失 × 4 种子 × TTA
  - 样本级随机划分 80/10/10
  - Cosine LR, GaussNLL warmup=10ep

使用方法:
    python hetero_ensemble_v5_temporal.py
"""
...
SAVE_DIR = EXPERIMENTS_DIR / "PortMoEv2-Hetero-Ensemble-v5-temporal"
...
def train_single_seed(seed, d, device):
    ...
    print(f"\n{'=' * 50}")
    print(f"  Seed {seed} — v5 时序训练 ({CFG['epochs']} epochs, k={TEMPORAL_K})")
    print(f"{'=' * 50}")
...
def ensemble_evaluate(seeds, d, device):
    ...
    print(f"\n{'=' * 60}")
    print(f"  v5 时序集成评估 ({len(seeds)} seeds × Hetero × TTA × k={TEMPORAL_K})")
    print(f"{'=' * 60}")
    ...
    print(f"v4 (增强+SWA):             Test MAE_TEU=5.1137  R²=0.9539")
    print(
        f"v5 (时序k={TEMPORAL_K}):             "
...
def main():
    ...
    print("=" * 60)
    print(f"  PortMoEv2-Hetero Ensemble v5 — 时序增强 (k={TEMPORAL_K})")
    print("=" * 60)
